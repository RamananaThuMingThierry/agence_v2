...
export default function ContactFormsPage() {
  ...
  return (
    <div className="space-y-2">
      <section className="overflow-hidden rounded-sm bg-white/90">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">Consultez les demandes recues depuis le site et supprimez celles qui ne sont plus utiles.</p>
          </div>
        </div>

        <div className="mt-3 grid gap-4 md:grid-cols-2">
          <div className="rounded-sm border border-stone-200 bg-white px-5 py-4 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Total</p><p className="mt-2 text-3xl font-extrabold text-slate-950">{contactForms.length}</p></div>
          <div className="rounded-sm border border-stone-200 bg-white px-5 py-4"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Avec sujet</p><p className="mt-2 text-3xl font-extrabold text-slate-950">{contactForms.filter((item) => Boolean(item.subject)).length}</p></div>
        </div>
        ...
        <div className="mt-3 overflow-hidden rounded-sm border border-stone-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Rechercher une demande..." className="w-full rounded-sm border border-stone-300 bg-white px-4 py-3 text-sm text-slate-700 outline-none transition sm:w-80" />
              <button type="button" onClick={loadContactForms} className="inline-flex items-center justify-center rounded-sm border border-stone-300 bg-white px-5 py-3 text-sm font-bold text-slate-700 transition hover:border-red-300 hover:text-red-800">Rafraichir</button>
            </div>
          </div>
          ...
          <div className="mt-3">
            {loading ? (
              ...
              <EmptyState title="Aucune demande trouvee" description="Ajustez la recherche ou verifiez si de nouveaux formulaires ont ete recus." />
            ) : (
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-stone-200">
                    <thead className="bg-stone-50 text-left">
                      <tr>
                        <th className="px-5 py-4 text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Nom / Email</th>
                    ...
                    <tbody className="divide-y divide-stone-200 bg-white">
                      {filteredContactForms.map((contactForm) => (
                        <tr key={contactForm.encrypted_id || contactForm.id} className="align-top">
                          <td className="px-5 py-4"><div><p className="font-extrabold text-slate-950">{contactForm.name}</p><p className="mt-1 text-sm text-slate-500">{contactForm.email}</p></div></td>
                          <td className="px-5 py-4 text-sm font-semibold text-slate-700">{contactForm.subject || "-"}</td>
                          <td className="px-5 py-4 text-sm text-slate-600"><p className="max-w-xl line-clamp-3">{contactForm.message}</p></td>
                          <td className="px-5 py-4 text-sm text-slate-500">{contactForm.created_at ? new Date(contactForm.created_at).toLocaleDateString("fr-FR") : "-"}</td>
                          <td className="px-5 py-4"><div className="flex flex-wrap justify-end gap-2"><Link to={`/admin/contact-forms/${contactForm.encrypted_id}`} className="rounded-sm border px-4 py-2 text-sm font-bold text-slate-700 transition hover:bg-black hover:text-white">Details</Link><button type="button" onClick={() => openConfirm(contactForm)} className="rounded-sm border border-rose-200 px-4 py-2 text-sm font-bold text-rose-700 transition hover:bg-red-600 hover:text-white">Supprimer</button></div></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      <ConfirmModal open={Boolean(confirmState)} title={confirmState?.title || ""} message={confirmState?.message || ""} confirmText={confirmState?.confirmText || "Confirmer"} loading={actionLoading} onCancel={() => (actionLoading ? undefined : setConfirmState(null))} onConfirm={handleDelete} />
    </div>
  );
}
